...
inline void init() {
    _rrep(i,N*80-1,1) stk.emplace(i);
}
...
inline void insert(int& u,int l,int r,int p) {
    ...
    if (l==p && r==p) {
        ...
        int mid=l+r>>1;
        if (p<=mid) insert(tr[u].l,l,mid,p);
        else insert(tr[u].r,mid+1,r,p);
    }
}
...
inline void solve(int l,int r,vector<int>& vec) {
    if (vec.empty()) return;
    int mid=l+r>>1;
    _rep(i,l,mid+1) cnt[i]=0;
    _rrep(i,mid,l) {
        cnt[i]=cnt[i+1];
        int pos=UB(R[i],mid);
        cnt[i]+=pos;
    }
    vector<int> lvec,mvec,rvec;
    _for(i,vec) {
        if (y[i]<=mid) lvec.emplace_back(i);
        else if (x[i]>=mid+1) rvec.emplace_back(i);
        else mvec.emplace_back(i);
    }
    int cur=mid;
    int croot=0;
    _for(i,mvec) {
        while (cur<y[i]) {
            ++cur;
            merge(croot,root[cur]);
        }
        ans[i]=query(croot,1,n,x[i],y[i])+cnt[x[i]];
    }
    solve(l,mid,lvec);
    solve(mid+1,r,rvec);
}
...
int main() {
    ...
    while (T--) {
        read(n);
        _rep(i,1,n) read(arr[i].l,arr[i].r),R[i].clear(),L[i].clear(),insert(root[arr[i].r],1,n,arr[i].l);
        read(m);
        _rep(i,1,m) read(x[i],y[i]);
        _rep(i,1,n) R[arr[i].l].emplace_back(arr[i].r),L[arr[i].r].emplace_back(arr[i].l);
        vector<int> st;
        _rep(i,1,m) st.emplace_back(i);
        sort(_all(st),[&](const int& a,const int& b) {
            return y[a]<y[b];
        });
        solve(1,n,st);
        _rep(i,1,m) writeln(ans[i]);
        _rep(i,1,n) reset(root[i]);
    }
    return 0;
}
...
inline void dfs(int x,int y) {
    ...
    _rep(p,0,7) {
        ...
        if (!vis[nx][ny] && arr[nx][ny]==arr[x][y]) dfs(nx,ny);
    }
}
...
int main() {
    ...
    while (T--) {
        read(n,m);
        _rep(i,0,n+m+5) pos[i].clear();
        _rep(i,1,n) _rep(j,1,m) vis[i][j]=0;
        _rep(i,1,n) _rep(j,1,m) {
            read(arr[i][j]);
            if (arr[i][j]<=n+m+5) pos[arr[i][j]].emplace_back(vi2{i,j});
        }
        _rep(t,0,n+m+5) {
            bool ok=true;
            if (arr[1][1]==t) ok=false;
            else if (arr[n][m]==t) ok=false;
            else {
                _for(p,pos[t]) {
                    int x=p.a,y=p.b;
                    if (vis[x][y]) continue;
                    fleft=0,fright=0,fup=0,fdown=0;
                    dfs(x,y);
                    if (fleft && fup) ok=false;
                    if (fleft && fright) ok=false;
                    if (fup && fdown) ok=false;
                    if (fright && fdown) ok=false;
                    if (!ok) break;
                }
            }
            if (ok) {

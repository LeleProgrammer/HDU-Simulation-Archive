...
namespace chunithm {
    ...
    inline void build() {
        _rep(i,1,n) st[i][0]=b[i];
        _rep(j,1,K-1) _rep(i,1,n) {
            st[i][j]=st[i][j-1];
            if (i+(1<<(j-1))<=n) chmax(st[i][j],st[i+(1<<(j-1))][j-1]);
        }
    }
...
namespace sega {
    ...
    inline void build() {
        _rep(i,1,n) st[i][0]=a[i];
        _rep(j,1,K-1) _rep(i,1,n) {
            st[i][j]=st[i][j-1];
            if (i+(1<<(j-1))<=n) st[i][j]=gcd(st[i][j],st[i+(1<<(j-1))][j-1]);
        }
    }
...
int main() {
    ...
    while (T--) {
        ...
        while (q--) {
            ...
            while (cur>=l) {
                // _rrep(i,K-1,0) {
                //     if (cur-(1<<i)<l) continue;
                //     int g=sega::query(cur-(1<<i),r);
                //     if (g==las) cur-=(1<<i);
                // }
                int L=l,R=cur;
                while (L<R) {
                    int mid=L+R>>1;
                    int g=sega::query(mid,r);
                    if (g==las) R=mid;
                    else L=mid+1;
                }
                cur=R;
            ...
            _rep(i,0,(int)vec.size()-1) {
                int pos=vec[i];
                int g=sega::query(pos,r);
                int rpos=r;
                if (i!=(int)vec.size()-1) rpos=vec[i+1]-1;
                i64 mx=chunithm::query(pos,rpos)-dec;
                if (mx<=0) continue;
                // cout<<"process: "<<g<<" "<<mx<<" "<<dec<<endl;
                i64 cnt=mx/g;
                if (mx%g) cnt++;
                ans+=cnt;
                dec+=cnt*g;

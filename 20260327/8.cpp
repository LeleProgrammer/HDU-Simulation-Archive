...
inline void pushdown(val& u,val& l,val& r) {
    if (u.add) {
        l.add+=u.add; l.mn+=u.add;
        r.add+=u.add; r.mn+=u.add;
        u.add=0;
    }
}

inline void modify(val& u,const int& k) {
    if (u.mn==inf) u.mn=0;
    u.mn+=k,u.add+=k;
}

SEG<val,pushup,pushdown,int,modify> seg;
...
i32 main() {
    ...
    while (T--) {
        read(n);
        seg.reserve((n<<1)+5);
        _rep(i,1,n) read(a[i]),a[n+i]=a[i];
        _rep(i,1,n<<1) p[i]=p[i-1]+a[i];
        _rep(i,1,n<<1) seg.modify(1,i,i,p[i]);
        _rep(i,1,n) {
            // cout<<"test: "<<i<<": "<<seg.query(1,i,i+n-1).mn<<endl;
            if (seg.query(1,i,i+n-1).mn>=0) {
                writeln(i);
                break;
            }
            seg.modify(1,i+1,n<<1,-a[i]);
        }
    }
    return 0;
}
...
i32 main() {
    ...
    while (T--) {
        read(n);
        _rep(i,1,9) vec[i].clear();
        _rep(i,1,n) {
            int a,b,v;
            read(a,b,v);
            vec[a*b].emplace_back(v);
        }
        _rep(i,1,9) sort(_all(vec[i]),[&](const int& a,const int& b) {
            return a>b;
        });
        _rep(i,1,9) _rep(j,1,(int)vec[i].size()-1) vec[i][j]+=vec[i][j-1];
        ans=sum=0;
        dfs(9,9);
        writeln(ans);
    }

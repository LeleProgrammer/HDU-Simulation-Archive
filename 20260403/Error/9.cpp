...
inline void pushup(int u) {
    tr[u].mn=min(tr[u].l?tr[tr[u].l].mn:inf,tr[u].r?tr[tr[u].r].mn:inf);
    tr[u].val=min(tr[u].l?tr[tr[u].l].val:inf,tr[u].r?tr[tr[u].r].val:inf);
}
...
inline void pushdown(int u) {
    ...
    if (root.dec) {
        left.dec+=root.dec;
        left.val-=root.dec*(1LL<<left.d);
        chmin(left.mn,left.val>>left.d);
        right.dec+=root.dec;
        right.val-=root.dec*(1LL<<right.d);
        chmin(right.mn,right.val>>right.d);
        root.dec=0;
    }
    if (root.dd) {
        left.dd+=root.dd;
...
inline void decr(int p,int u,int l,int r,int L,int R,int k) {
    tr[u]=tr[p];
    if (l>=L && r<=R) {
        tr[u].dec+=k;
        tr[u].val-=k*(1LL<<tr[u].d);
        // cout<<"d: "<<tr[u].d<<endl;
        chmin(tr[u].mn,tr[u].val>>tr[u].d);
    } else {
        pushdown(u);
        int mid=l+r>>1;
        if (L<=mid) decr(tr[p].l,tr[u].l=++idx,l,mid,L,R,k);
        if (R>mid) decr(tr[p].r,tr[u].r=++idx,mid+1,r,L,R,k);
        pushup(u);
    }
}

inline void rmov(int p,int u,int l,int r,int L,int R) {
    tr[u]=tr[p];
    if (l>=L && r<=R) {
        tr[u].dd++;
        tr[u].d++;
        ...
        pushdown(p);
        int mid=l+r>>1;
        if (L<=mid) rmov(tr[p].l,tr[u].l=++idx,l,mid,L,R);
        if (R>mid) rmov(tr[p].r,tr[u].r=++idx,mid+1,r,L,R);
        pushup(u);
    }
}
    ...
    while (T--) {
        ...
        while (q--) {
            ...
            if (op==1) {
                int x;
                read(x);
                decr(las,nex,1,n,l,r,x);
            } else {
                rmov(las,nex,1,n,l,r);
            }
            // cout<<"test: mn: "<<tr[nex].mn<<" "<<tr[nex].val<<endl;
            if (tr[nex].mn<=m) las=root,ans++;
            else las=nex;
        }
        writeln(ans);
    }

...
i32 main() {
    ...
    while (T--) {
        ...
        read(n,m);
        _rep(i,1,n) read(a[i]);
        _rep(i,1,n+1) read(b[i]);
        sort(a+1,a+1+n,[&](const i128& a,const i128& b) {
            return a>b;
        });
        sort(b+1,b+1+n+1,[&](const i128& a,const i128& b) {
            return a>b;
        });
        i128 sum=0;
        _rep(i,1,n) sum+=a[i]*b[i];
        if (sum>=m) puts("0");
        else {
            _rep(i,1,n) pre[i]=pre[i-1]+a[i]*b[i];
            _rrep(i,n,1) suf[i]=suf[i+1]+a[i]*b[i+1];
            i128 ans=inf;
            _rep(i,1,n+1) {
                i128 cur=suf[i]+pre[i-1];
                i128 rem=m-cur;
                i128 res=rem/b[i];
                if (rem%b[i]) res++;
                chmin(ans,res);

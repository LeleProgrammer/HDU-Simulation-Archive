...
i32 main() {
    ...
    while (T--) {
        read(n,m,k);
        _rep(i,0,m-1) pre[i].clear(),suf[i].clear();
        _rep(i,0,n-1) read(a[i]);
        _rep(i,0,n-1) pre[i%m].emplace_back(a[i]);
        _rrep(i,n-1,0) suf[(n-1-i)%m].emplace_back(a[i]);
        _rep(i,0,m-1) {
            _rep(j,1,(int)pre[i].size()-1) pre[i][j]+=pre[i][j-1];
            _rep(j,1,(int)suf[i].size()-1) suf[i][j]+=suf[i][j-1];
        }
        int ans=pre[k-1].back();
        _rep(i,0,n-2) {
            // cout<<"test: "<<i<<": ";
            int t=n-1-(i+k);
            int sum=0;

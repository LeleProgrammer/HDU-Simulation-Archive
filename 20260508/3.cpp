...
int main() {
    ...
    while (T--) {
        read(n);
        _rep(k,1,n) {
            _rep(i,0,n) f[0][i]=f[1][i]=0;
            f[0][0]=1;
            _rep(i,1,n) _rep(j,0,k) {
                f[i&1][j]=0;
                if (j) (f[i&1][j]+=f[i&1^1][j-1])%=mod;
                if (j) (f[i&1][j]+=f[i&1^1][j]*2%mod)%=mod;
                else (f[i&1][j]+=f[i&1^1][j])%=mod;
                if (j!=k) (f[i&1][j]+=f[i&1^1][j+1])%=mod;
            }
            writesp((f[n&1][0]+f[n&1][1])%mod);
        }
        putchar(10);
    }
    return 0;

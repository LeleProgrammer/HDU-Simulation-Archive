...
constexpr inline int calc(int x) {
    return x*(x+1)/2;
}
...
i32 main() {
    read(T);
    while (T--) {
        _rep(i,0,N-1) pre[i]=suf[i]=ansl[i]=ansr[i]=0;
        read(n,k);
        _rep(i,1,n) read(a[i]);
        ...
        if (k>=(n+1)/2) {
            int ext=k-(n+1)/2;
            int l=1,r=n,ans=0,cur=1;
            while (l<=r) {
                if (l!=r) ans+=a[l]+ext+cur+a[r]+ext+cur;
                else ans+=a[l]+ext+cur+cur-1;
                cur++;
                l++,r--;
            }
            writeln(ans);
            ...
            _rep(i,1,n) {
                int j=max(0LL,i-k);
                int len=i-j;
                ansl[i]=pre[i]-pre[j]+calc(k)-calc(k-len);
                chmax(ansl[i],ansl[i-1]);
            }
            _rrep(i,n,1) {
                int j=min(n+1,i+k);
                int len=j-i;
                ansr[i]=suf[i]-suf[j]+calc(k)-calc(k-len);
                chmax(ansr[i],ansr[i+1]);
            }
            int ans=0;
            _rep(i,0,n) chmax(ans,ansl[i]+ansr[i+1]);
            writeln(ans);
        }
    }
    return 0;

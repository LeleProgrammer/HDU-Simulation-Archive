...
int main() {
    ...
    while (T--) {
        ...
        mpr.clear();
        mpc.clear();
        _rep(i,1,n) _rep(j,1,n) mpr[a[i][j]].emplace(i);
        _rep(i,1,n) _rep(j,1,n) mpc[a[i][j]].emplace(j);
        vector<int> ans;
        _for(t,mpr) {
            int num=t.first;
            if (mpr[num].size()==n && mpc[num].size()==n) ans.emplace_back(num);
        }
        writeln(ans.size());
        if (!ans.empty()) {
            _for(g,ans) writesp(g);
            putchar(10);
        }
    }
    return 0;

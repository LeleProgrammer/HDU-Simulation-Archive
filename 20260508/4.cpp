...
i32 main() {
    ...
    while (T--) {
        ...
        _rep(i,0,1) _rep(j,0,N-1) _rep(k,0,2) f[i][j][k]=-inf;
        f[0][0][0]=0;
        _rep(i,1,n) {
            _rep(j,0,N-1) _rep(k,0,2) f[i&1][j][k]=-inf;
            _rep(j,0,i) {
                if (j) {
                    chmax(f[i&1][j][1],f[i&1^1][j-1][0]-a[i]);
                    chmax(f[i&1][j][2],max(f[i&1^1][j-1][1]-a[i],f[i&1^1][j-1][2]-a[i]));
                }
                if (j<i) {
                    chmax(f[i&1][j][0],max(f[i&1^1][j+1][1]+a[i],f[i&1^1][j+1][0]+a[i]));
                    if (a[i-1]<a[i-2]) chmax(f[i&1][j][0],f[i&1^1][j+1][2]+a[i]);
                }
            }
        }
        int ans=-inf;
        _rep(i,0,n) chmax(ans,max3(f[n&1][i][0],f[n&1][i][1],f[n&1][i][2])+i*a[n+1]);
        writeln(ans);
    }
    return 0;
}
...
inline i64 query(int l,int r,i64 x,i64 y) {
    if (B[l]==B[r]) {
        i64 ans=-inf;
        _rep(i,l,r) chmax(ans,min((i64)arr[i].a+x,(i64)arr[i].b+y));
        return ans;
    }
    i64 ans=-inf;
    _rep(i,l,R[l]) chmax(ans,min((i64)arr[i].a+x,(i64)arr[i].b+y));
    _rep(i,L[r],r) chmax(ans,min((i64)arr[i].a+x,(i64)arr[i].b+y));
    _rep(i,B[l]+1,B[r]-1) {
        int le=BL[i]-1,ri=BR[i];
        while (le<ri) {
            int mid=(le+ri>>1)+1;
            if (st[mid].a-st[mid].b<y-x) le=mid;
            else ri=mid-1;
        }
        if (le!=BL[i]-1) chmax(ans,(i64)pre[le]+x);
        if (le!=BR[i]) chmax(ans,(i64)suf[le+1]+y);
    }
    return ans;
}
...
int main() {
    read(T);
    while (T--) {
        read(n),read(q),len=sqrt(n);
        _rep(i,1,n) read(arr[i].a),read(arr[i].b),st[i]=arr[i];
        _rep(i,1,n) B[i]=get(i);
        _rep(i,1,n) if (i==1 || B[i]!=B[i-1]) L[i]=i; else L[i]=L[i-1];
        _rrep(i,n,1) if (i==n || B[i]!=B[i+1]) R[i]=i; else R[i]=R[i+1];
        _rep(i,1,n) BL[B[i]]=L[i],BR[B[i]]=R[i],Bn=B[i];
        _rep(i,1,Bn) sort(st+BL[i],st+BR[i]+1);
        _rep(t,1,Bn) {
            pre[BL[t]]=st[BL[t]].a;
            _rep(i,BL[t]+1,BR[t]) pre[i]=max(pre[i-1],st[i].a);
            suf[BR[t]]=st[BR[t]].b;
            _rrep(i,BR[t]-1,BL[t]) suf[i]=max(suf[i+1],st[i].b);
        }
        i64 las=0;
        while (q--) {
            int l,r;

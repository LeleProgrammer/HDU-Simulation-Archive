...
int main() {
    ...
    while (T--) {
        nums.clear();
        _rep(i,1,N-1) vec[i].clear(),p[i]=i;
        read(n);
        _rep(i,1,n) read(a[i]),nums.emplace_back(a[i]);
        read(m);
        _rep(i,1,m) {
            read(op[i]);
            if (op[i]==1) read(x[i],y[i]),nums.emplace_back(x[i]),nums.emplace_back(y[i]);
            else read(x[i]),nums.emplace_back(x[i]);
        }
        _sort(nums),_unique(nums);
        _rep(i,1,n) a[i]=LB(nums,a[i])+1;
        _rep(i,1,m) {
            x[i]=LB(nums,x[i])+1;
            if (op[i]==1) y[i]=LB(nums,y[i])+1;
        }
        _rep(i,1,n) vec[p[a[i]]].emplace_back(i);
        _rep(i,1,m) {
            if (op[i]==1) {
                bool flag=false;
                if (vec[p[x[i]]].size()>vec[p[y[i]]].size()) {
                    swap(x[i],y[i]);
                    flag=true;
                }
                while (!vec[p[x[i]]].empty()) {
                    int u=vec[p[x[i]]].back(); vec[p[x[i]]].pop_back();
                    vec[p[y[i]]].emplace_back(u);
                }
                if (flag) swap(p[x[i]],p[y[i]]);
            } else if (op[i]==2) {
                vec[p[x[i]]].emplace_back(++n);
            } else if (op[i]==3) {
                vec[p[x[i]]].clear();
            }
        }
        memset(ans,-1,sizeof(ans));
        _rep(i,1,N-1) _for(u,vec[p[i]]) ans[u]=i;
        _rep(i,1,N-1) if (~ans[i]) writesp(nums[ans[i]-1]);
        putchar(10);
    }
    return 0;
}
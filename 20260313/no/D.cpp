...
inline void init() {
    _rep(i,2,P-1) {
        bool flag=true;
        _rep(j,2,i-1) if (i%j==0) {
            flag=false;
            break;
        }
        if (flag) pre[i]=i,num[++idx]=i,ind[i]=idx;
        else pre[i]=pre[i-1];
    }
}
...
//     else return f[i][j][k]=dfs((i+(1<<k))%n,dfs(i,j,k-1),k-1);
// }
...
int main() {
    ...
    while (T--) {
        memset(f,-1,sizeof(f));
        read(n);
        _rep(i,0,n-1) read(a[i]);
        _rep(i,0,n-1) _rep(j,0,30) f[i][j][0]=ind[pre[num[j]+a[(i+1)%n]]];
        // _rep(i,0,2) _rep(j,0,5) {
        //     printf("f[%d][%d][0] = %d\n",i,num[j],num[f[i][j][0]]);
        // }
        // _rep(k,1,K-1) _rep(i,0,n-1) _rep(j,0,30) f[i][j][k]=f[(i+(1<<k))%n][f[i][j][k-1]][k-1];
        _rep(i,0,n-1) _rep(j,0,30) f[i][j][1]=f[(i+(1<<1))%n][f[i][j][1-1]][1-1];
        _rep(i,0,n-1) _rep(j,0,30) f[i][j][2]=f[(i+(1<<2))%n][f[i][j][2-1]][2-1];
        _rep(i,0,n-1) _rep(j,0,30) f[i][j][3]=f[(i+(1<<3))%n][f[i][j][3-1]][3-1];
        _rep(i,0,n-1) _rep(j,0,30) f[i][j][4]=f[(i+(1<<4))%n][f[i][j][4-1]][4-1];
        _rep(i,0,n-1) _rep(j,0,30) f[i][j][5]=f[(i+(1<<5))%n][f[i][j][5-1]][5-1];
        _rep(i,0,n-1) _rep(j,0,30) f[i][j][6]=f[(i+(1<<6))%n][f[i][j][6-1]][6-1];
        _rep(i,0,n-1) _rep(j,0,30) f[i][j][7]=f[(i+(1<<7))%n][f[i][j][7-1]][7-1];
        _rep(i,0,n-1) _rep(j,0,30) f[i][j][8]=f[(i+(1<<8))%n][f[i][j][8-1]][8-1];
        _rep(i,0,n-1) _rep(j,0,30) f[i][j][9]=f[(i+(1<<9))%n][f[i][j][9-1]][9-1];
        _rep(i,0,n-1) _rep(j,0,30) f[i][j][10]=f[(i+(1<<10))%n][f[i][j][10-1]][10-1];
        _rep(i,0,n-1) _rep(j,0,30) f[i][j][11]=f[(i+(1<<11))%n][f[i][j][11-1]][11-1];
        _rep(i,0,n-1) _rep(j,0,30) f[i][j][12]=f[(i+(1<<12))%n][f[i][j][12-1]][12-1];
        _rep(i,0,n-1) _rep(j,0,30) f[i][j][13]=f[(i+(1<<13))%n][f[i][j][13-1]][13-1];
        _rep(i,0,n-1) _rep(j,0,30) f[i][j][14]=f[(i+(1<<14))%n][f[i][j][14-1]][14-1];
        _rep(i,0,n-1) _rep(j,0,30) f[i][j][15]=f[(i+(1<<15))%n][f[i][j][15-1]][15-1];
        _rep(i,0,n-1) _rep(j,0,30) f[i][j][16]=f[(i+(1<<16))%n][f[i][j][16-1]][16-1];
        int ans=0;
        _rep(i,0,n-1) {
            int m=n,cur=0,pos=i;
            _rrep(j,K-1,0) if ((1<<j)<=m) cur=f[pos][cur][j],pos=(pos+(1<<j))%n,m-=(1<<j);
            // _rrep(j,K-1,0) if ((1<<j)<=m) cur=dfs(pos,cur,j),pos=(pos+(1<<j))%n,m-=(1<<j);
            chmax(ans,num[cur]);
        }
        writeln(ans);
    }

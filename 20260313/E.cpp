...
i32 main() {
    ...
    while (T--) {
        read(n,k,m),m++;
        _rep(i,1,n) read(a[i]),p[i]=p[i-1]+a[i];
        _rep(i,n+1,n+60) a[i]=0,p[i]=p[i-1];
        _rep(i,0,n+60) _rep(j,0,k+1) f[i][j]=-inf;
        f[0][0]=0;
        _rep(i,1,n+60) {
            f[i][0]=max(f[i-1][0]+a[i],f[i-1][k+1]+a[i]);
            if (i-m>=0) {
                _rep(j,max(0LL,k-m+2),k+1) chmax(f[i][0],f[i-m][j]+p[i]-p[i-m]);
            }
            f[i][1]=max(f[i-1][0],f[i-1][k+1])+a[i]*2;
            _rep(j,2,k+1) {
                f[i][j]=f[i-1][j-1]+a[i]/2;
                if (i-m>=0 && j-m>=0) chmax(f[i][j],f[i-m][j-m]+p[i]-p[i-m]);
            }
        }
        writeln(f[n+60][0]);
    }

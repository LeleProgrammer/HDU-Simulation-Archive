...
inline bool check(int mid) {
    Graph::restore_graph(n*m+5);
    _rep(i,1,n) _rep(j,1,m) if (t[get(i,j)]<=mid) {
        _rep(p,0,3) {
            int nx=i+dx4[p];
            int ny=j+dy4[p];
            if (nx<1 || ny<1 || nx>n || ny>m || t[get(nx,ny)]>mid) continue;
            add(get(i,j),get(nx,ny),0);
        }
    }
    _rep(i,1,k) if (t[get(arr[i].x,arr[i].y)]<=mid && t[get(arr[i].p,arr[i].q)]<=mid) add(get(arr[i].x,arr[i].y),get(arr[i].p,arr[i].q),arr[i].w);
    SP::restore(n*m+5);
    dijkstra(get(1,1));
    return dis[get(n,m)]<=lmt;
}
...
i32 main() {
    ...
    while (T--) {
        read(n,m,lmt,k);
        _rep(i,1,n) _rep(j,1,m) read(t[get(i,j)]);
        _rep(i,1,k) read(arr[i].x,arr[i].y,arr[i].p,arr[i].q,arr[i].w);
        int l=0,r=1e9;
        while (l<r) {
            int mid=l+r>>1;
            if (check(mid)) r=mid;

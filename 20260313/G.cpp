...
int main() {
    ...
    while (T--) {
        read(n);
        _rep(i,1,n) read(a[i]);
        _rep(i,0,n+1) f[i]=g[i]=-inf;
        _rep(i,1,n) f[i]=max(f[i-1]+a[i],a[i]);
        _rrep(i,n,1) g[i]=max(g[i+1]+a[i],a[i]);
        _rep(i,1,n) chmax(f[i],f[i-1]);
        _rrep(i,n,1) chmax(g[i],g[i+1]);
        int ans=-inf;
        _rep(i,3,n-2) chmax(ans,f[i-2]+a[i]+g[i+2]);
        writeln(ans);
    }
    return 0;
}
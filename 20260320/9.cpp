...
inline void bfs() {
    _rep(i,1,n) if (X[i]>=-200 && Y[i]>=-200 && X[i]<=200 && Y[i]<=200) {
        dis[X[i]+P][Y[i]+P]=1;
        q.emplace(pii{X[i]+P,Y[i]+P});
    }
    while (!q.empty()) {
        ...
        _rep(i,1,n) {
            ...
            if (nx>=-200+P && ny>=-200+P && nx<=200+P && ny<=200+P && dis[nx][ny]>dis[x][y]+1) {
                dis[nx][ny]=dis[x][y]+1;
                q.emplace(pii{nx,ny});
            }
        }
...
int main() {
    ...
    while (T--) {
        ...
        _rep(i,0,N-1) _rep(j,0,N-1) dis[i][j]=inf;
        read(n);
        _rep(i,1,n) read(X[i],Y[i]);
        bfs();
        int ans=dis[P][P];
        if (ans==inf) puts("-1");
        else writeln(ans);

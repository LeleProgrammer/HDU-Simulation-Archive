...
char s[N],t[N];
int st[N];
unordered_map<u64,int,_custom_hash> hs[N];

int main() {
    read(T);
    while (T--) {
        memset(st,0,sizeof(st));
        _rep(i,0,N-1) hs[i].clear();
        readstr(s+1);
        readstr(t+1);
        n=strlen(s+1);
        _rep(i,1,n) {
            int l=i,r=i;
            st[l]++;
            while (l-1>=1 && r+1<=n && s[l-1]==s[r+1]) l--,r++,st[l]++;
        }
        _rep(i,1,n-1) if (s[i]==s[i+1]) {
            int l=i,r=i+1;
            st[l]++;
            while (l-1>=1 && r+1<=n && s[l-1]==s[r+1]) l--,r++,st[l]++;
        }
        ...
        _rep(i,1,n) {
            ...
            _rep(j,i,n) {
                cur=cur*P+s[j];
                if (!hs[j-i+1].count(cur)) continue;
                ans+=(st[j+1]+1)*hs[j-i+1][cur];
            }
        }
        // =====
        memset(st,0,sizeof(st));

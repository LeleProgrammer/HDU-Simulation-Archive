...
i32 main() {
    ...
    while (T--) {
        vec.clear();
        read(n,m);
        _rep(i,1,m) read(arr[i].x,arr[i].y),arr[i].x--;
        sort(arr+1,arr+1+m);
        int las=inf;
        bool ok=true;
        _rep(i,1,m) {
            ...
            if (arr[i].x) vec.emplace_back(arr[i]);
            if (arr[i].x==0 && arr[i].y!=n) {
                ok=false;
                break;
            }
            las=arr[i].y;
        ...
        _rep(i,0,(int)vec.size()-1) {
            cnt+=cur-vec[i].y; cur=vec[i].y-1;
            int pos=vec[i].x-lasp+1; lasp=vec[i].x+1;
            if (cnt+1<pos) {
                ok=false;
                break;
            }
            // cout<<"cnt, pos: "<<cnt<<" "<<pos<<endl;
            ans=ans*pos%mod*A(cnt,pos-1)%mod;
            cnt-=pos-1;
        }
        if (!ok) {
            puts("0");

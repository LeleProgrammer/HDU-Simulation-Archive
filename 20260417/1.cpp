...
i32 main() {
    ...
    while (T--) {
        read(n,q);
        readstr(s+1);
        _rep(i,0,n+1) suf[i]=pre[i]=cnt[i]=0;
        _rep(i,1,n) pre[i]=pre[i-1]+i*(s[i]=='-');
        _rrep(i,n,1) suf[i]=suf[i+1]+i*(s[i]=='+');
        _rep(i,1,n) cnt[i]=cnt[i-1]+(s[i]=='+');
        int res=0;
        while (q--) {
            int l,r;
            read(l,r);
            ...
            if (s[p]=='+') {
                int L=pre[p-1]-pre[l-1];
                int R=suf[p+1]-suf[r+1];
                int ans=R*2-L*2+p-(l-1);
                // cout<<"test: "<<l<<" "<<p<<" "<<r<<" + "<<L<<" "<<R<<endl;
                // writeln(ans);
                res^=ans;
            } else {
                int L=pre[p-1]-pre[l-1];

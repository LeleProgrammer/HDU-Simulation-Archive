...
inline void modify(int u,int p,int k) {
    if (tr[u].l==p && tr[u].r==p) tr[u].mn=k;
    else {
        int mid=tr[u].l+tr[u].r>>1;
...
inline int query(int u,int l,int r) {
    ...
    int ans=inf;
    int mid=tr[u].l+tr[u].r>>1;
    if (l<=mid) chmin(ans,query(u<<1,l,r));
    if (r>mid) chmin(ans,query(u<<1|1,l,r));
    return ans;
}
...
i32 main() {
    ...
    while (T--) {
        ...
        _rep(i,2,n) {
            int l=max(1LL,i-k-1);
            int mn=query(1,l,i-1);
            modify(1,i,mn+a[i]);
        }
        writeln(sum+query(1,n-k,n));
    }
    return 0;
}
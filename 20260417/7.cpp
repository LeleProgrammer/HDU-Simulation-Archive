...
struct Yoisaki_Kanade_is_my_wife {
    int l,r;
    int add;
    int set;
} tr[N<<2];
...
inline void pushdown(int u) {
    ...
    if (root.add) {
        if (!~left.set) left.add+=root.add;
        else ans[left.set]+=root.add*(left.r-left.l+1);
        if (!~right.set) right.add+=root.add;
        else ans[right.set]+=root.add*(right.r-right.l+1);
        root.add=0;
    } else if (~root.set) {
        if (left.add) pushdown(u<<1);
        if (right.add) pushdown(u<<1|1);
        left.set=root.set;
        right.set=root.set;
        root.set=-1;
    }
...
inline void modify1(int u,int l,int r,int x) {
    if (tr[u].l>=l && tr[u].r<=r) {
        if (!~tr[u].set) tr[u].add+=x;
        else ans[tr[u].set]+=x*(tr[u].r-tr[u].l+1);
    } else {
        pushdown(u);
        int mid=tr[u].l+tr[u].r>>1;
...
inline void modify2(int u,int l,int r,int x) {
    if (tr[u].l!=tr[u].r) pushdown(u);
    if (tr[u].l>=l && tr[u].r<=r) tr[u].set=x;
    else {
        pushdown(u);
        int mid=tr[u].l+tr[u].r>>1;
...
inline void pushall(int u,int l,int r) {
    if (l==r) return;
    pushdown(u);
    int mid=l+r>>1;
    pushall(u<<1,l,mid);
    pushall(u<<1|1,mid+1,r);
}
...
i32 main() {
    ...
    while (T--) {
        ...
        while (q--) {
            int op,l,r,x;
            read(op,l,r,x);
            if (op==1) modify2(1,l,r,x);
            else modify1(1,l,r,x);
        }
        pushall(1,1,n);
        _rep(i,1,m) writesp(ans[i]);
        putchar(10);
    }
    return 0;
}
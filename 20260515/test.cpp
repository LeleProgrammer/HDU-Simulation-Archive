...
int brute(vector<pair<int,int>>& segs) {
    int n = segs.size();
    vector<int> cur(n);
    int sum = 0, cnt = 0;
    function<void(int)> dfs = [&](int idx) {
        if (idx == n) {
            for (int i = 0; i < n-1; i++) sum += abs(cur[i] - cur[i+1]);
            cnt++;
            return;
        }
        for (int v = segs[idx].first; v <= segs[idx].second; v++) {
            cur[idx] = v;
            dfs(idx+1);
        }
    };
    dfs(0);
    return sum % mod * Math::inverse(cnt) % mod;
}

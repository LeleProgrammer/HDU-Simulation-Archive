...
i32 main() {
    ...
    while (T--) {
        ...
        read(n,m);
        int M=0;
        _rep(i,0,n-1) read(a[i]),cnta[a[i]]++,chmax(M,a[i]);
        _rep(i,0,m-1) read(b[i]),cntb[b[i]]++,chmax(M,b[i]);
        _rep(i,1,M) {
            sa[i]=cnta[i]*i%mod;
            sb[i]=cntb[i]*i%mod;
        }
        _rep(i,1,M) _reps(j,i*2,M,j+=i) {
            (sa[i]+=sa[j])%=mod;
            (sb[i]+=sb[j])%=mod;
        }
        _rep(i,1,M) f[i]=Math::inverse(i*i);
        _rep(i,1,M) _reps(j,i*2,M,j+=i) f[j]=(f[j]-f[i]+mod)%mod;
        int ans=0;
        _rep(d,1,M) (ans+=f[d]*sa[d]%mod*sb[d]%mod)%=mod;
        writeln(ans);
    }
    return 0;
}
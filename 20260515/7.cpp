...
int T,n;
int l[N],r[N];
int inv2=Math::inverse(2);
int inv3=Math::inverse(3);
int inv6=Math::inverse(6);

constexpr inline int calc(int l,int r) {
    return (l+r)*(r-l+1)%mod*inv2%mod;
}

constexpr inline int calc2(int fr,int st,int cnt) {
    fr%=mod,st%=mod,cnt%=mod;
    (fr+=mod)%=mod;
    (st+=mod)%=mod;
    (cnt+=mod)%=mod;
    int ed=fr+st*(cnt-1);
    ed%=mod;
    return (fr+ed)*cnt%mod*inv2%mod;
}

constexpr inline int calc3(int g) {
    return g*(g+1)%mod*(g+2)%mod*inv6%mod;
    // g%=mod;
    // int ans=g*(g+1)%mod*(g*2+1)%mod*inv6%mod;
    // (ans+=calc(1,g))%=mod;
    // (ans*=inv2)%=mod;
    // return ans;
}

constexpr inline int S1(int a,int m,int x,int y){
    if(a>=m)return 0;
    int len=m-a;
    int sum_v=(x+y)%mod*((y-x)%mod)%mod*inv2%mod;
    int term1=sum_v*(len%mod)%mod;
    int sum_u=(a+m)%mod*(len%mod)%mod*inv2%mod;
    int term2=sum_u*((y-x)%mod)%mod;
    return (term1-term2)%mod;
}
constexpr inline int S2(int x, int M, int y){
    if(x >= M) return 0;
    int d = M - x;
    int L = y - x;
    int t1 = d % mod * d % mod * d % mod * inv3 % mod;  // 2/3 * d^3 * 1/2 = d^3/3
    int t2 = L % mod * d % mod * d % mod * inv2 % mod;   // L * d^2 * 1/2
    int t3 = L % mod * L % mod * d % mod * inv2 % mod;   // L^2 * d * 1/2
    return (t1 - t2 + t3) % mod;
}
constexpr inline int S3(int y,int b,int x,int y0){
    if(y>=b)return 0;
    int len=b-y;
    int sum_v=(x+y0)%mod*((y0-x)%mod)%mod*inv2%mod;
    int sum_u=(y+b)%mod*(len%mod)%mod*inv2%mod;
    int term1=sum_u*((y0-x)%mod)%mod;
    int term2=sum_v*(len%mod)%mod;
    return (term1-term2)%mod;
}
...
i32 main() {
    ...
    while (T--) {
        ...
        int sum=0;
        int tot=1;
        _rep(i,1,n) if (r[i] > l[i]) (tot*=(r[i]-l[i]))%=mod;
        // _rep(i,1,n) (tot*=(r[i]-l[i]+1))%=mod;
        _rep(i,1,n-1) {
            if(r[i] == l[i] || r[i+1] == l[i+1]) continue;
            int cnt=tot*Math::inverse(r[i]-l[i])%mod*Math::inverse(r[i+1]-l[i+1])%mod;
            int a=l[i],b=r[i];
            int x=l[i+1],y=r[i+1];
        ...
        // _rep(i,1,n-1) {
        //     int cnt=tot*Math::inverse(r[i]-l[i]+1)%mod*Math::inverse(r[i+1]-l[i+1]+1)%mod;
        //     int a=l[i],b=r[i];
        //     int x=l[i+1],y=r[i+1];
        //     if (a>x) swap(a,x),swap(b,y);
        //     int t=min(x-1,b);
        //     int g=calc(1,y-x+1);
        //     // cout<<"test: "<<g<<endl;
        //     (sum+=calc2(g,y-x+1,x-a)*cnt)%=mod;
        //     (sum-=calc2(g,y-x+1,x-(t+1))*cnt)%=mod;
        //     if (b>y) (sum+=calc2(g,y-x+1,b-y)*cnt)%=mod;
        //     int L=x,R=min(b,y);
        //     if (L<=R) {
        //         // cout<<"add: "<<calc3(R-L)<<" "<<calc3(y-L)<<" "<<calc3(y-R-1)<<endl;
        //         (sum+=calc3(R-L)*cnt)%=mod;
        //         (sum+=calc3(y-L)*cnt)%=mod;
        //         (sum-=calc3(y-R-1)*cnt)%=mod;
        //     }
        //     // cout<<"now: "<<sum<<endl;
        // }
        sum=(sum%mod+mod)%mod;
        (sum*=Math::inverse(tot))%=mod;
        writeln(sum);
    }
    return 0;
}

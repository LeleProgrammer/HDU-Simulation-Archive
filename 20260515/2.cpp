...
inline int solve() {
    if (r.empty()) {
        ...
        int ans=inf;
        _iter(it,g) {
            if (next(it)==g.end()) break;
            chmin(ans,max((*it)-mng,mxg-*(next(it))));
        }
        return ans;
    }
    int mnr=*(r.begin());
    int mxr=*prev(r.end());
    if (g.empty()) return mxr-mnr;
    vector<int> toera;
    _for(t,g) if (t>=mnr && t<=mxr) toera.emplace_back(t);
    _for(t,toera) g.erase(t);
    if (g.empty()) return mxr-mnr;
    int mng=*(g.begin());
    int mxg=*prev(g.end());
    int cur=max(mxr-mnr,mxg-mng);
    if (mng>mxr || mxg<mnr) return cur;
    br=r,bg=g;
    _iter(it,bg) {
        if (next(it)==bg.end()) break;
        br.emplace(*it);
        chmin(cur,max(*prev(br.end())-*(br.begin()),*prev(bg.end())-*next(it)));
    }
    br=r,bg=g;
    _riter(it,bg) {
        if (next(it)==bg.rend()) break;
        br.emplace(*it);
        chmin(cur,max(*prev(br.end())-*(br.begin()),*next(it)-*prev(bg.rend())));
    }
    return cur;
}
...
int main() {
    ...
    while (T--) {
        ...
        if (!vr.empty()) {
            ...
            mxr=*prev(vr.end());
            if (r.size()>=2) {
                int smnr=*next(vr.begin());
                int smxr=*prev(prev(vr.end()));
                vector<int> toera;
                _iter(t,vg) {
                    int v=*t;
                    if (v>=smnr && v<=smxr) toera.emplace_back(v);
                }
                _for(t,toera) vg.erase(t);
            }
        }
        int mxg,mng;
        if (!vg.empty()) {

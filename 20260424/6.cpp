...
inline void bfs(int s) {
    _rep(i,0,N-1) dis[i]=inf,flag[i]=0;
    q.emplace(s); dis[s]=0;
    while (!q.empty()) {
        int u=q.front(); q.pop();
        if (flag[u]) continue;
        flag[u]=true;
        _graph(i,u) if (!flag[e[i]] && dis[e[i]]>dis[u]+1) {
            dis[e[i]]=dis[u]+1;
            q.emplace(e[i]);
        }
    }
...
int main() {
    ...
    while (m--) {
        ...
        scanf(" %c",&c);
        if (c=='(') {
            _rep(i,0,n-1) add(get(u,i),get(v,i+1),1);
            _rep(i,0,n-1) add(get(v,i),get(u,i+1),1);
        } else {
            _rep(i,1,n) add(get(u,i),get(v,i-1),1);
            _rep(i,1,n) add(get(v,i),get(u,i-1),1);
        }
    }
    _rep(i,1,n) {
        bfs(get(i,0));
        _rep(j,1,n) d[i][j]=dis[get(j,0)];
    }
    while (qq--) {
        int x,y;
        read(x,y);

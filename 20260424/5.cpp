...
int main() {
    ...
    while (T--) {
        ...
        vec.clear();
        _rep(i,1,n) {
            if (vec.empty()) vec.emplace_back(pii{a[i],1});
            else if (vec.back().first==a[i]) vec.back().second++;
            else vec.emplace_back(pii{a[i],1});
            if (!vec.empty() && vec.back().second==k) vec.pop_back();
        }
        int len=0;
        _for(g,vec) len+=g.second;
        writeln(len);
        _for(g,vec) _rep(i,1,g.second) writesp(g.first);
        putchar(10);
    }
    return 0;
}
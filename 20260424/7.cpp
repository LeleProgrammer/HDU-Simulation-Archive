...
int main() {
    ...
    while (T--) {
        st.clear();
        _rep(i,1,n) while (!pos[a[i]].empty()) pos[a[i]].pop_front();
        read(n,m);
        _rep(i,1,n) read(a[i]),pos[a[i]].emplace_back(i);
        bool ok=true;
        _rep(i,1,m) if (pos[i].empty()) {
            ok=false;
            break;
        }
        if (!ok) {
        ...
        _rep(i,1,m) st.emplace(pos[i].front());
        int ans=solve();
        int cur=0;
        while (cur<n) {
            cur++;
            int k=a[cur];
            if (k>m) continue;
            st.erase(pos[k].front());
            pos[k].pop_front();
            if (pos[k].empty()) break;
            st.emplace(pos[k].front());
            chmin(ans,solve());
        }
        writeln(ans);
    }

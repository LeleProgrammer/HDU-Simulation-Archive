...
inline void init() {
    ...
    do {
        int k=0;
        _rep(i,0,5) k=k*10+vec[i];
        ind[k]=++idx;
        num[idx]=k;
    } while (next_permutation(_all(vec)));
}

constexpr inline int calc(int a,int b,int c,int d,int e,int f) {
    return a*100000+b*10000+c*1000+d*100+e*10+f;
}

constexpr inline int goUp(int a,int b,int c,int d,int e,int f) {
    return calc(b,f,c,d,a,e);
}

constexpr inline int goDown(int a,int b,int c,int d,int e,int f) {
    return calc(e,a,c,d,f,b);
}

constexpr inline int goLeft(int a,int b,int c,int d,int e,int f) {
    return calc(c,b,f,a,e,d);
}

constexpr inline int goRight(int a,int b,int c,int d,int e,int f) {
    return calc(d,b,a,f,e,c);
}

inline void solve(int st) {
    _rep(i,1,n) _rep(j,1,m) _rep(k,1,idx) {
        dis[i][j][k]=inf;
        flag[i][j][k]=0;
    }
    dis[sx][sy][ind[st]]=0;
    q.emplace(vi3{sx,sy,ind[st]});
    while (!q.empty()) {
        auto t=q.front(); q.pop();
        int x=t.a,y=t.b,p=t.c;
        if (flag[x][y][p]) continue;
        flag[x][y][p]=true;
        int g=num[p];
        int f=g%10; g/=10;
        int e=g%10; g/=10;
        ...
        int a=g;
        if (x>1) {
            int nex=ind[goUp(a,b,c,d,e,f)];
            if (dis[x-1][y][nex]>dis[x][y][p]+1) {
                dis[x-1][y][nex]=dis[x][y][p]+1;
                q.emplace(vi3{x-1,y,nex});
            }
        }
        if (x<n) {
            int nex=ind[goDown(a,b,c,d,e,f)];
            if (dis[x+1][y][nex]>dis[x][y][p]+1) {
                dis[x+1][y][nex]=dis[x][y][p]+1;
                q.emplace(vi3{x+1,y,nex});
            }
        ...
        if (y>1) {
            int nex=ind[goLeft(a,b,c,d,e,f)];
            if (dis[x][y-1][nex]>dis[x][y][p]+1) {
                dis[x][y-1][nex]=dis[x][y][p]+1;
                q.emplace(vi3{x,y-1,nex});
            }
        }
        if (y<m) {
            int nex=ind[goRight(a,b,c,d,e,f)];
            if (dis[x][y+1][nex]>dis[x][y][p]+1) {
                dis[x][y+1][nex]=dis[x][y][p]+1;
                q.emplace(vi3{x,y+1,nex});
            }
...
int main() {
    ...
    while (T--) {
        ...
        _rep(i,1,idx) {
            ...
            int b=g%10; g/=10;
            int a=g;
            if (f==dest) chmin(ans,dis[tx][ty][i]);
        }
        if (ans==inf) ans=-1;
        writeln(ans);
    }
